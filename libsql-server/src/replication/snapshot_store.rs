...
struct SnapshotStoreInner {
    ...
    conn: RwLock<libsql_sys::Connection<Sqlite3Wal>>,
}
...
impl SnapshotStoreInner {
    fn register(
        ...
        snapshot_id: Uuid,
        exec_before: impl FnOnce() -> crate::Result<()>,
        ) -> crate::Result<()> {
        assert!(dbg!(start_frame_no) <= dbg!(end_frame_no));
        let mut conn = self.conn.write();
        let txn = conn.transaction()?;
        exec_before()?;
        {
            let mut stmt = txn.prepare_cached("INSERT INTO snapshots VALUES (?, ?, ?, ?, date(), 0)")?;
            stmt.execute((namespace.as_str(), start_frame_no, end_frame_no, snapshot_id.to_string()))?;
        }
        txn.commit()?;
        Ok(())
    }
...
impl SnapshotStore {
    pub async fn new(base_path: &Path) -> crate::Result<Self> {
        ...
        let conn = open_conn_enable_checkpoint(&store_path, Sqlite3WalManager::default(), None, 1000)?;
        ...
        let inner = Arc::new(SnapshotStoreInner { snapshots_path, temp_path, conn: RwLock::new(conn) });
    ...
    pub fn find(&self, namespace: &NamespaceName, frame_no: FrameNo) -> crate::Result<Option<Uuid>> {
        let conn = self.inner.conn.read();
        let mut stmt = conn.prepare_cached(
            ...
            LIMIT 1"#)?;
        let snapshot_id = stmt.query_row(named_params!( ":namespace": namespace.as_str(), ":fno": frame_no), |row| {
    ...
    pub fn find_file(&self, namespace: &NamespaceName, frame_no: FrameNo) -> crate::Result<Option<SnapshotFile>> {
        match self.find(namespace, frame_no)? {
            Some(snapshot_id) => {
                let path = self.inner.snapshots_path().join(snapshot_id.to_string());
                let file = tokio::runtime::Handle::current().block_on(SnapshotFile::open(path))?;
                Ok(Some(file))
            }
            None => Ok(None),
        }
...
impl SnapshotBuilder {
    pub fn add_frame(&mut self, frame: &FrameBorrowed) -> crate::Result<()> {
        ...
        match self.last_seen_frame_no {
            ...
            None => {
                self.end_frame_no = dbg!(frame_no);
            }
        }
    ...
    pub fn finish(mut self, start_frame_no: FrameNo) -> crate::Result<()> {
        ...
        let snapshot_path = self.store.snapshots_path().join(snapshot_id.to_string());
        ...
        let ret = self.store.register(&self.name, start_frame_no, end_frame_no, snapshot_id, || {
...
mod test {
    ...
    async fn bigger_snapshot_is_returned() {
        ...
        store.inner.register(&name, 1, 17, id_small, || Ok(())).unwrap();
        let id_big = Uuid::new_v4();
        store.inner.register(&name, 1, 35, id_big, || Ok(())).unwrap();
        assert_eq!(store.find(&name, 1).unwrap().unwrap(), id_big);

        // try again but inserting the big first
        let id_big = Uuid::new_v4();
        store.inner.register(&name, 46, 175, id_big, || Ok(())).unwrap();
        let id_small = Uuid::new_v4();
        store.inner.register(&name, 42, 61, id_small, || Ok(())).unwrap();
